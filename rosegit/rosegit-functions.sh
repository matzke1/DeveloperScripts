...
rosegit_environment () {
    [ -d "$RG_SRC" ] || rosegit_die "no source directory"
    [ -d "$RG_BLD" ] || rosegit_die "no build directory"
    ...
    LD_LIBRARY_PATH="$LD_LIBRARY_PATH:$RG_BLD/src/.libs"
    LD_LIBRARY_PATH="$LD_LIBRARY_PATH:$RG_BLD/libltdl/.libs"
    LD_LIBRARY_PATH="$LD_LIBRARY_PATH:$RG_BLD/src/3rdPartyLibraries/libharu-2.1.0/src/.libs"
    LD_LIBRARY_PATH="$LD_LIBRARY_PATH:$RG_BLD/src/3rdPartyLibraries/qrose/QRoseLib/.libs"
...
#    RG_BLD -- the name of the top of the build tree, set to the absolute name of the current working directory
#    RG_SRC -- the name of the top of the source tree corresponding to this build tree.
#    ROSE_SRC    -- the name of the top of the source tree for ROSE (same as RG_SRC if we're compiling ROSE itself)
rosegit_find_directories () {
    [ -n "$RG_BLD" ] || RG_BLD=$(rosegit_find_builddir)
    ...
    export RG_BLD=$(cd $RG_BLD && pwd)
    export RG_SRC=$(cd $RG_SRC && pwd)

    # Find the ROSE source tree
    [ -f "$RG_SRC/src/rose.h" ] && ROSE_SRC=$RG_SRC
    [ -z "$ROSE_SRC" -a -f "$RG_SRC/config/ROSE_SOURCES" ] && ROSE_SRC=$(cat $RG_SRC/config/ROSE_SOURCES)
...
# Loads configuration files.  All variables starting with "RG_" are exported; others are only exported if done so
...
rosegit_load_config () {
    ...
    confdirs="$confdirs:$mydir/config:$repo/scripts/rosegit/config"
    ...
    [ -n "$RG_LOADED" ] || rosegit_die "default config should have set RG_LOADED"
...
#    RG_CONFIGURE=$(echo "$RG_CONFIGURE" |tr '\n' ' ')
}

# Runs RG_MAKE. Used by test scripts.
rosegit_make () {
    eval "$RG_MAKE" "$@"
}

# Echoes the current namespace based on the setting of the RG_NAMESPACE variable.  If this variable is empty then we
...
# Returns success/failure depending on whether the specified argument is a recognized rosegit switch
...
rosegit_preamble () {
	...
	RG_BLD="$blddir"
	RG_SRC="$srcdir"
	RG_NAMESPACE="$namespace"
	RG_USE_CMAKE="$use_cmake"
	rosegit_find_directories
	local branch=$(cd $RG_SRC && git branch 2>/dev/null |sed -n '/^\*/s/^\* //p')
	...
	rosegit_environment
	echo "config=$config" >"$RG_BLD/.rosegit"

    elif [ -n "$blddir" -a "$blddir" != "$RG_BLD" ] || \
	 [ -n "$srcdir" -a "$srcdir" != "$RG_SRC" ] || \
	 [ -n "$namespace" -a "$srcdir" != "$RG_NAMESPACE" ] || \
	 [ -n "$config" ]; then
...
rosegit_show_environment () {
    ...
    echo "Source tree:       $RG_SRC"
    echo "Build tree:        $RG_BLD"
    [ "$RG_SRC" != "$ROSE_SRC" ] && echo "ROSE source tree:  $ROSE_SRC"
    ...
    echo "Autoconf:"
    eval "perl -e 'print qq{    \$_\n} for sort {(split q{=},\$a)[0] cmp (split q{=},\$b)[0]} @ARGV' -- $RG_CONFIGURE"
    echo "CMake:"
    eval "perl -e 'print qq{    \$_\n} for sort {(split q{=},\$a)[0] cmp (split q{=},\$b)[0]} @ARGV' -- $RG_CMAKE"
}
